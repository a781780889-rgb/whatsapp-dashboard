...
const QUICK_ACTIONS = [
  ...
  { icon: Lightbulb,    label: 'وضع الاقتراح',   key: 'suggest',     color: 'text-amber-400',   bg: 'bg-amber-400/10',   border: 'border-amber-400/20'   },
...
function SysCard({ sys, isExpanded, onToggle }: {
  sys: SystemCardData;
  isExpanded: boolean;
  onToggle: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <IconComponent className={cn('w-5 h-5', sys.color)} />
        </div>
      ...
      {isExpanded && (
        <div className="px-5 pb-5 pt-1 border-t border-[var(--border-default)]">
          ...
          <div className="mt-4 flex gap-2">
            <button className="flex-1 py-2 rounded-xl text-xs font-bold border bg-[var(--brand-primary)]/10 text-[var(--brand-primary)] border-[var(--brand-primary)]/25 hover:bg-[var(--brand-primary)]/20 transition-all">
              إعدادات النظام
            </button>
            <button className="flex-1 py-2 rounded-xl text-xs font-bold border bg-[var(--bg-elevated)] text-[var(--text-secondary)] border-[var(--border-default)] hover:text-[var(--text-primary)] transition-all">
              عرض التقرير
            </button>
          </div>
        </div>
...
export default function AIAutomationView() {
  ...
  const [toggles, setToggles] = useState<ToggleState>({
    ai_on: true, auto_reply: true, watch_mode: false,
    suggest: true, learn_mode: true, ai_off: false,
    train: false, retrain: false, export: false, import: false,
  });
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in" dir="rtl">
      ...
      <div className="relative flex items-center justify-between px-5 py-4 rounded-2xl overflow-hidden border border-[var(--border-default)] bg-gradient-to-l from-violet-500/6 via-[var(--bg-surface)] to-[var(--bg-surface)]">
        ...
        <div className="flex items-center gap-2 bg-[var(--bg-elevated)] border border-[var(--border-default)] px-3 py-1.5 rounded-full">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          ...
          { icon: Brain,    label: 'في وضع التعلم',   value: String(learningCount),                color: 'text-violet-400',  bg: 'bg-violet-400/10'  },
              ...
              {QUICK_ACTIONS.map(action => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold border transition-all duration-200 hover:brightness-110',
            ...
            <div className="flex flex-col gap-1.5">
              {ADVANCED_TOOLS.map((tool, i) => {
                ...
                return (
                  ...
                  >
                    <ToolIcon className="w-4 h-4 text-[var(--brand-primary)] shrink-0" />
                    <span className="flex-1 truncate">{tool.label}</span>
                    {tool.badge && (
                      <span className={cn(
                        'text-[10px] font-bold px-1.5 py-0.5 rounded-md',
                        tool.badge === 'جديد' ? 'bg-emerald-400/15 text-emerald-400' : 'bg-amber-400/15 text-amber-400'
                      )}>
                        {tool.badge}
                      </span>
                    )}
      ...
      <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
          {[
            { range: '1 – 130',   label: 'المرحلة الأساسية',       desc: 'الأنظمة الثمانية + الأدوات المتقدمة',         pct: 100, status: 'done',    color: '#00A884' },
            { range: '131 – 200', label: 'إدارة المعرفة المؤسسية', desc: 'أنظمة إدارة المعرفة على مستوى المؤسسة',       pct: 45,  status: 'active',  color: '#4F8EF7' },
            { range: '201 – 300', label: 'التحليل التنبؤي',        desc: 'توصيات ذكية وتحليل متقدم للبيانات',           pct: 10,  status: 'soon',    color: '#a78bfa' },
            { range: '301 – 500', label: 'وكلاء الذكاء المستقل',  desc: 'وكلاء AI متخصصون ومراقبة ذاتية للأداء',      pct: 0,   status: 'planned', color: '#f59e0b' },
          ].map((phase, i) => (
            <div key={i} className="flex flex-col gap-3 p-4 rounded-xl border border-[var(--border-default)] bg-[var(--bg-elevated)]">
              ...
              <div className="h-1 rounded-full bg-[var(--bg-overlay)] overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${phase.pct}%`, background: phase.color }} />
              </div>
      ...
      <div className="flex items-start gap-3 px-5 py-4 rounded-2xl border border-amber-500/20 bg-amber-500/5">
        ...
        <div>
          ...
          <p className="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
            الوكلاء الذكيون والتحليل التنبؤي المتقدم (المرحلة 201-500) قيد التطوير. سيتم إطلاقها تدريجياً خلال 2026-2027.
            تأكد من تهيئة متغيرات البيئة <code className="font-mono text-amber-400 bg-amber-400/10 px-1 rounded">AI_PROVIDER</code> و <code className="font-mono text-amber-400 bg-amber-400/10 px-1 rounded">ANTHROPIC_API_KEY</code> لتفعيل المحرك.
          </p>
        </div>
      </div>
