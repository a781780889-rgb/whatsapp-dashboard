...
class UserController {
    ...
    async create(req, res) {
        try {
            ...
            // ── المرحلة 2: استخدام Transaction لضمان Atomicity ───────────────
            ...
            await TransactionManager.withTransaction(async (client) => {
                ...
                // Create subscription inside same transaction
                const subId = uuidv4();
                const planHours = this._planHours(planType);
                const expiresAt = planHours
                    ? new Date(Date.now() + planHours * 3600000).toISOString()
                    : null;
                await client.query(
                    `INSERT INTO subscriptions (id,user_id,plan_type,expires_at,status,created_by)
                     VALUES ($1,$2,$3,$4,'active',$5)`,
                    [subId, id, planType || 'monthly', expiresAt, req.user.id]
                );

                // Issue license inside same transaction
                await client.query(
                    `INSERT INTO licenses (id,user_id,license_key,status,issued_by)
                     VALUES ($1,$2,$3,'active',$4)`,
                    [uuidv4(), id, licKey, req.user.id]
                );
            });
