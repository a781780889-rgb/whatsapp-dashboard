...
class SystemDB {
    ...
    async init() {
        ...
        // ── المرحلة 2: Database Hardening Migrations ────────────────────────
        const MigrationV2 = require('./MigrationV2');
        await MigrationV2.run().catch(err =>
            console.warn('[SystemDB] MigrationV2 non-fatal error:', err.message)
        );
    }
    ...
    async withTransaction(fn) {
        const { withTransaction } = require('./TransactionManager');
    ...
    // ── Refresh Tokens ────────────────────────────────────────────────────────
    async saveRefreshToken(userId, tokenHash, ip, userAgent, expiresAt, family = null) {
        await query(
            `INSERT INTO refresh_tokens (id, user_id, token_hash, ip, user_agent, expires_at, family)
             VALUES ($1,$2,$3,$4,$5,$6,$7)
             ON CONFLICT (token_hash) DO NOTHING`,
            [crypto.randomUUID(), userId, tokenHash, ip, userAgent, expiresAt, family]
        );
    }
    ...
    // [FIX-13] إبطال كل tokens تنتمي لـ family محدد (Token Theft Response)
    async revokeAllUserTokensByFamily(family) {
        if (!family) return;
