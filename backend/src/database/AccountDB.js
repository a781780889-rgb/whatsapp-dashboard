...
class AccountDB {
    ...
    async init() {
        ...
        try {
            ...
            // ── المرحلة 2: إضافة الـ Indexes الناقصة ─────────────────────────────
            await this._addV2Indexes(client);
    ...
    // ── V2 Indexes: إضافة indexes الأداء المفقودة ────────────────────────────
    async _addV2Indexes(client) {
        const indexes = [
            // wa_groups — البحث بالـ JID والاسم
            `CREATE INDEX IF NOT EXISTS idx_wa_groups_name
                ON wa_groups(name)`,
            `CREATE INDEX IF NOT EXISTS idx_wa_groups_publish
                ON wa_groups(publish_status)`,
            `CREATE INDEX IF NOT EXISTS idx_wa_groups_sync
                ON wa_groups(last_sync DESC)`,

            // messages — استرجاع سريع للمحادثات
            ...
            // campaigns — تصفية حالة الحملات
            `CREATE INDEX IF NOT EXISTS idx_campaigns_status
                ON campaigns(status, created_at DESC)`,

            // extracted_links — تصفية بالنوع والتاريخ
            `CREATE INDEX IF NOT EXISTS idx_links_type_status
                ON extracted_links(link_type, status)`,
            `CREATE INDEX IF NOT EXISTS idx_links_extracted
                ON extracted_links(extracted_at DESC)`,

            // auto_join_queue — معالجة الطابور
            `CREATE INDEX IF NOT EXISTS idx_join_queue_status_sched
                ON auto_join_queue(status, scheduled_at)`,

            // broadcast_schedules — إن وُجد الجدول
            `CREATE INDEX IF NOT EXISTS idx_bcast_status
                ON broadcast_schedules(status)`,
        ];

        for (const sql of indexes) {
            await client.query(sql).catch(() => {}); // Silently skip if exists or table absent
        }
    }
    ...
    // ── Transaction Helper (المرحلة 2) ────────────────────────────────────────
    async withTransaction(fn) {
        const TransactionManager = require('./TransactionManager');
        return TransactionManager.withAccountTransaction(this, fn);
    }
